...
object SimplifyProd2 {
  ...
  def addFactor(factors: List[ArithExpr], factor: ArithExpr,
                ...
                someFactorsComeFromPow: Boolean = true): ArithExpr = {
    ...
    val simplifiedOriginalProd: ArithExpr =
      if (factors.length > 1) {
        ...
        if (factorsComeFromProd) new Prod(factors)
        else {
  ...
  def apply(lhs: ArithExpr, rhs: ArithExpr): ArithExpr = {
    if (PerformSimplification()) {
      ...
      new Prod(List(lhs, rhs).sortWith(ArithExpr.isCanonicallySorted))
  }
